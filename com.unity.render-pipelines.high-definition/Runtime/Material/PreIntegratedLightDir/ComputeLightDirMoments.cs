...
{
    class ComputeLightDirMoments
    {
        Material      m_ComputeLightDirMomentsMaterial;

        Matrix4x4[]   m_faceWorldToViewMatrixMatrices     = new Matrix4x4[6];

        RenderPipelineResources m_RenderPipelineResources;

        public ComputeLightDirMoments(RenderPipelineResources renderPipelineResources)
        {
        ...
        {
            return m_ComputeLightDirMomentsMaterial != null;
        }

        public void Initialize(CommandBuffer cmd)
        {
            if (m_ComputeLightDirMomentsMaterial == null)
            {
                m_ComputeLightDirMomentsMaterial = CoreUtils.CreateEngineMaterial(m_RenderPipelineResources.shaders.computeLightDirMomentsPS);
            }
        ...
        public void Cleanup()
        {
            CoreUtils.Destroy(m_ComputeLightDirMomentsMaterial);
            m_ComputeLightDirMomentsMaterial = null;
        }
        ...
        public void Compute(CommandBuffer cmd, Texture source, RenderTexture target1, RenderTexture target2, RenderTexture target3)
        {
            ...
            using (new ProfilingSample(cmd, "Compute Cubemap Moments"))
            {
                for (int face = 0; face < 6; ++face)
                {
                    ...
                    var transform = HDUtils.ComputePixelCoordToWorldSpaceViewDirectionMatrix(0.5f * Mathf.PI, Vector2.zero, faceSize, m_faceWorldToViewMatrixMatrices[face], true);
                    ...
                    CoreUtils.DrawFullScreen(cmd, m_ComputeLightDirMomentsMaterial, props);
